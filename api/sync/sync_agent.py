...
class SyncAgent:
    def __init__(
        ...
        min_n_interactions_threshold: int = 1,
        ...
        self.min_n_interactions_threshold = min_n_interactions_threshold
    ...
    # Background poll loop - runs for the lifetime of the app. (Called on app launch)
    async def run_loop(self):
        log.info("Sync agent started (min_N=%d N=%d, T=%ds)", self.min_n_interactions_threshold, self.n_interactions_threshold, self.t_seconds_threshold)
        while True:
            await asyncio.sleep(5)
            await self.maybe_trigger_upload()

    # Checks if the upload threshold has been reached.
    async def maybe_trigger_upload(self):
        ...
        if n_k >= self.min_n_interactions_threshold and (n_k >= self.n_interactions_threshold or elapsed >= self.t_seconds_threshold):
            await self.upload_backbone(reason="threshold_reached")

    # Checks for a newer backbone version from the server. (Called on app launch)
    async def check_for_newer_backbone(self):
        local_version = self.model_manager.backbone_version
        url = (
            f"{self.server_url}/backbone/model"
            f"?since={local_version}&algorithm={self.model_manager.algorithm}"
        )
        try:
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if resp.status_code == 200:
                    ...
                    if server_version != local_version:
                        log.info(
                            "Newer backbone available: %s → %s",
                            local_version,
                            server_version,
                        )
                        self.model_manager.load_global_backbone(
                            data["backbone_weights"],
                            server_version,
                        )
    ...
    # Uploads the backbone weights to the server. (Called on upload threshold reached)
                ...
                async with httpx.AsyncClient(timeout=30) as client:
                    resp = await client.post(
                        ...
                        headers={
                            ...
                            "X-Backbone-Version": str(self.model_manager.backbone_version),
                            "X-Reason": reason,
                        },
                    )
